...
1. **Code Format**: Specify the `edit_complete` variable in a single Python code snippet, strictly adhering to the specified format.

2. **Edit Completeness Priority**: Prioritize enabling the requested edit. As a secondary factor, also consider maintaining the shape's integrity. If either of these cannot be achieved, then the list of edits is incomplete. 

3. **No Backtracking**: Ignore if the edit on a part is not the correct one. Only consider if the part has been edited or not.

4. **Unedited Parts**: Remember that currently the unedited parts are properly supporting the shape's integrity. Only consider if the edit request explicitly requires them to be edited.
...
1. **Code Format**: Specify the new edit in a executable Python code snippet, strictly adhering to the specified API format. Remember to specify only a single new edit with the `new_edit` variable.

2. **Function Use**: Employ functions like `center` and `face_center` to define the primary edit precisely.

3. **Part Selection**: Use the `shape.get(label)` function to access the part with label `label`, where the label is the path from the root part to the part to be accessed (e.g., `left_front_leg = shape.get("legs/leg_front_left")`)

4. **Parts are Cuboids**: Remember that all the parts are cuboids.

5. **Scaling / Rotating Parts** When scaling or rotating parts, always carefully consider the value of the `origin` parameter.

6. **Global Directions**: Remember that all the directional variables "front", "back", "left", "right", "up", "down" are defined with respect to the global coordinate system. Therefore, the bottom face of the seat approximately faces the 'down' direction and the top face of the seat faces the 'up' direction approximately.
